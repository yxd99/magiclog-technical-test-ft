...
export const fetchMyProductsService = async (
...
export const createProductService = async (product: CreateProduct) => {
  const response = await httpClient.post<unknown, CreateProduct>(
    "/products",
    product,
  );
  return response.data;
...
export const updateProductService = async (
  ...
  const response = await httpClient.patch<Product, CreateProduct>(
    `/products/${productId}`,
    product,
  );
  return response.data;
